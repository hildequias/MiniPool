...
Ball::Ball (GameLayer * game, int type, Point position, int color) : b2Sprite (game, type) {
    _type = type;
...
Ball* Ball::create(GameLayer * game, int type, Point position, int color) {
    auto sprite = new Ball(game, type, position, color);
	if (sprite) {
        sprite->initBall();
		sprite->autorelease();
		return sprite;
